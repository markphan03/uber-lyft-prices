...
print("MAPE:", mean_absolute_percentage_error(y_test, y_pred_lr)*100)

# RMSLE safety: clip negative predictions to 0
y_pred_lr_clip = np.clip(y_pred_lr, 0, None)
print("RMSLE:", root_mean_squared_log_error(y_test, y_pred_lr_clip))
...
# RMSLE safety: clip negative predictions to 0
y_pred_rf_clip = np.clip(y_pred_rf, 0, None)
print("RMSLE:", root_mean_squared_log_error(y_test, y_pred_rf_clip))
...
with torch.no_grad():
    preds = model(X_test_t).detach().cpu().numpy().flatten()
    y_true = y_test_t.detach().cpu().numpy().flatten()
...
rmsle = root_mean_squared_log_error(y_true_clip, preds_clip)
...
def bootstrap_rmse_ci(y_true, y_pred, n_bootstrap=500, random_state=42):
    ...
    return mean_rmse, lower, upper


lr_mean, lr_lower, lr_upper = bootstrap_rmse_ci(y_test, y_pred_lr)
rf_mean, rf_lower, rf_upper = bootstrap_rmse_ci(y_test, y_pred_rf)
nn_mean, nn_lower, nn_upper = bootstrap_rmse_ci(y_true, preds)

print("\n95% Confidence Interval for RMSE")
print(f"Linear Regression: {lr_mean:.4f} ({lr_lower:.4f} – {lr_upper:.4f})")
print(f"Random Forest: {rf_mean:.4f} ({rf_lower:.4f} – {rf_upper:.4f})")
print(f"Neural Network: {nn_mean:.4f} ({nn_lower:.4f} – {nn_upper:.4f})")
